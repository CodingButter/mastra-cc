...
# E2 renames to notes.txt, NOT to a name this reset creates. The first collection
# used receipt.txt - which E5 needs and this reset therefore writes - so E2's
# destination already existed before the errand began and the row measured nothing.
# The transcript now ends with a listing of $WORK so the rename is checked against
# the filesystem instead of against the agent's account of it.
reset_fixtures() {
  session_exec "mkdir -p $WORK && rm -f $WORK/proof.txt $WORK/receipt.txt $WORK/list.txt $WORK/notes.txt && printf 'the desk remembers this line\\n' > $WORK/proof.txt"
...
for errand in "${ERRANDS[@]}"; do
  for run in $(seq 1 "$RUNS"); do
    ...
    # The desk's own account of itself, after the fact. An errand that claims a file
    # was renamed is checkable here and nowhere else in the transcript.
    {
      echo
      echo "-- $WORK after the run, read from the desk itself --"
      session_exec "ls -1 $WORK" 2>/dev/null || echo "(listing unavailable)"
    } >> "$transcript"
