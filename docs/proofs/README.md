...
- [benchmark](benchmark/README.md): the 25-run cold-agent completion gate - 10/25 pass, zero daemon-class refusals, every failure owned by the world or the agent. Host-only (minibeast, GNOME/Wayland).
- [benchmark/webtop](benchmark/webtop/README.md): the same benchmark in the KDE Webtop container, with web tasks in visible Chromium over the accessibility route - 29/30 pass, zero daemon-class refusals; the earlier headless-CDP Webtop run is kept, labelled.
