...
| Tooling | `tools/mutations.mjs` intermittently reports "no readable report" (missing `report.json`) for one entry per full sweep; the entry passes when rerun alone | autonomous | Two full sweeps on Sep 17 each flaked on a different entry (`driver-releases-before-quiescence`, `watch-probe-failure-leaves-pending-work-live`); both caught in isolation. Likely vitest reporter/tmp race. |
