...
`without.txt.gz` retains the initial regression run against the old backend (13 failures, including incorrect Unicode and replacement refusals). `with.txt.gz` records the final executable proof. `workspace.txt.gz` records the forced build/lint/typecheck/test run. `mutations.txt.gz` records focused diagnostic-removal and duplicate-input mutations.
