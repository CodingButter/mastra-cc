...
ROOT="$(cd "$HERE/../../../../.." && pwd)"
RUN="$2"; CONSUMER="$3"; FIXTURE="$HERE/fixture.py"
if [[ "$1" == --display ]]; then
...
# The scale under test is chosen by the caller, not by this script, so one
# session file serves every factor. GDK_SCALE is X11's integer window scale;
# GDK_DPI_SCALE and Xft.dpi are the fractional text scale that moves widget
# geometry without moving the window scale - which is the case that can
# disagree with a rectangle read from the accessibility tree.
export GDK_SCALE="${CC07_GDK_SCALE:-1}" GDK_DPI_SCALE="${CC07_DPI_SCALE:-1}"
/usr/libexec/at-spi-bus-launcher --launch-immediately --a11y=1 >"$RUN/a11y.log" 2>&1 & pids+=("$!")
openbox >"$RUN/openbox.log" 2>&1 & pids+=("$!")
printf 'Xft.dpi: %s\n' "${CC07_XFT_DPI:-96}" | xrdb -merge - 2>>"$RUN/openbox.log" || true
sleep 1
...
mkfifo "$RUN/commands"
python3 "$FIXTURE" "$RUN/commands" >"$RUN/app.log" 2>&1 & pids+=("$!")
printf '%s\n' '--backend atspi --grant cc07-fixture --allow edit --allow activate --allow rawInput --audit audit.jsonl' >"$RUN/grant.txt"
node "$ROOT/daemon/dist/main.mjs" --backend atspi --grant cc07-fixture --allow edit --allow activate --allow rawInput --audit "$RUN/audit.jsonl" --socket "$RUN/runtime/d.sock" --ws-host 127.0.0.1 --ws-port 0 >"$RUN/daemon.log" 2>&1 & daemon=$!; pids+=("$daemon")
