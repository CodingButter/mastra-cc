...
printf '%s\n' '--backend atspi --grant cc01-fixture --allow edit --allow activate --allow rawInput --audit audit.jsonl' >"$RUN/grant.txt"
node "$ROOT/daemon/dist/main.mjs" --backend atspi --grant cc01-fixture --allow edit --allow activate --allow rawInput --audit "$RUN/audit.jsonl" --socket "$RUN/runtime/d.sock" --ws-host 127.0.0.1 --ws-port 0 >"$RUN/daemon.log" 2>&1 & daemon=$!; pids+=("$daemon")
