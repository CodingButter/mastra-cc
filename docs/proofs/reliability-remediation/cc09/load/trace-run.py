...
root = pathlib.Path(tempfile.mkdtemp(prefix='cc09-trace.'))
consumer = root / 'consumer'
consumer.mkdir()
os.symlink(modules, consumer / 'node_modules')
driver = pathlib.Path(__file__).with_name('trace-driver.mjs')
...
(root / 'declaration.json').write_text(json.dumps({
    ...
    'claim': 'native change-event trace under typing and replacement-burst workloads; one desk, no production claim',
}, indent=2))
