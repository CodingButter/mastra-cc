...
root = pathlib.Path(tempfile.mkdtemp(prefix='cc09-native.'))
consumer = root / 'consumer'
consumer.mkdir()
os.symlink(modules, consumer / 'node_modules')
driver = pathlib.Path(__file__).with_name('native-driver.mjs')
shutil.copyfile(driver, consumer / 'model-driver.mjs')
run = root / 'trial'
run.mkdir()
(run / 'before.txt').write_text('Native notification baseline\n')
(root / 'declaration.json').write_text(json.dumps({
    ...
    'sampleCount': 10, 'pollIntervalMs': 5,
    'claim': 'native mutation request to SQLite visibility upper bound; no cancellation claim',
}, indent=2))
...
with (run / 'session.txt').open('w') as log:
    proc = subprocess.Popen(['bash', str(session), '--display', str(run), str(consumer)],
        env={**os.environ, 'CC09_MASTRA_ENTRY': str(mastra_entry)},
