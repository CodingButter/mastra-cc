...
root = pathlib.Path(tempfile.mkdtemp(prefix='cc08-reparent.'))
...
(root / 'declaration.json').write_text(json.dumps({
    ...
    'claim': 'live GTK reparenting and root removal under a container watch; one fixture, one desk',
}, indent=2))
...
with (run / 'session.txt').open('w') as log:
    proc = subprocess.Popen(['bash', str(session), '--display', str(run), str(consumer)],
        env={**os.environ, 'CC08_MASTRA_ENTRY': str(mastra_entry)},
