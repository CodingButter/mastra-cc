...
printf '%s\n' '--backend atspi --grant reparent-fixture --allow edit --allow activate --allow rawInput' >"$RUN/grant.txt"
node "$ROOT/daemon/dist/main.mjs" --backend atspi --grant reparent-fixture --allow edit --allow activate --allow rawInput --socket "$RUN/runtime/d.sock" --ws-host 127.0.0.1 --ws-port 0 >"$RUN/daemon.log" 2>&1 & daemon=$!; pids+=("$daemon")
