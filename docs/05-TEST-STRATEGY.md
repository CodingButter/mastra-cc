...
- **B12 — every dependency carries a permissive licence.** Reads manifests rather than source, because a licence problem is never visible in source. Rule 3 below is the whole point of it. What it reads changed on 2026-08-21 (issue #36): a *runtime* dependency is followed to its own dependencies, and theirs, because the sentence that justifies reading manifests instead of source — deleting code does not delete the dependency — applies one level down as well. Not declaring a package does not mean not shipping it, and the first walk that could see them found three licences in this tree that the declared-list version never examined. Development dependencies are still read as declared: a build tool is not distributed, so its closure is a question about our build rather than about what a user receives, and widening to it would be a separate decision. Two further holes stay stated rather than pretended away — a system library we require but do not ship is out of its reach and needs a recorded exemption with a reason, and an abandoned-but-permissive project passes cleanly, which is why adoption records a maintenance note as well as a licence.
