...
**Language.** Python survives all three platforms — GObject introspection on Linux, COM interop on Windows, the Objective-C bridge on macOS all have established Python bindings. This is stated as design reasoning from binding availability, not from a build we have done. It is recorded because it means the cross-platform requirement does **not** reopen ADR-0010's language choice, which is the expensive thing it could have done.
...
2. **[ADR-0010](0010-daemon-is-python-single-threaded-default-glib-context.md) is re-scoped, not superseded.** Its threading, main-context, and venv rules are rules of the **Linux backend**. Its two-lane testing rule and its capability-probing rule are rules of **the daemon**. Its language choice stands.
