...
**Exit gate:**
- [x] Boundary pins B2, B3, B5 pass, and each **fails** under its mutation. **Ticked split, with B3's half stated rather than papered over** — see the footnote. Checkable: `node tools/pins/run.mjs` reports every wired pin green, and `node tools/mutations.mjs --table <scratch>` reddens a test per pin guard. (Ticked 2026-08-21: `pins: all 6 wired pin(s) green` — B2 wired in this milestone, `pin-b2: ok - 16 file(s) and 10 declared dependencies, no audio in the hub`, B5 `33 file(s), no socket client outside packages/transport`; a four-entry scratch run of `b2-source-half-removed`, `b2-manifest-half-removed`, `b2-vacuous-guard-removed` and `b5-vacuous-guard-removed` took exactly one test red each and reported `mutations: ok - 4 mutation(s), none survived`; PR #40.) **B3 is ticked on its runtime half only.** Its subject is a client holding a credential and this milestone ships no client, so a source pin scanning `apps/*` minus the hub would scan an empty file set and trip its own vacuity guard — it stays on the absence list in [tools/pins/README.md](../tools/pins/README.md) with that reason. The behaviour the boundary exists for *is* wired and tested: a credential presented by a caller is ignored and cannot rescue a dial the hub has no key for (`apps/hub/src/__tests__/the-voice-lane-mints-one-token.test.ts`). Its lasting guard is a type-surface assertion, `expect(lane.dial.length).toBe(0)` — `dial()` declares no parameters, so an honest caller has nowhere to put a credential, and the assertion reddens the moment anyone adds one. It carries no mutation entry because the runner only deletes and honouring a presented credential is an *addition*; that direction was bitten by hand once, which is a measurement rather than a standing guard — wiring the presented key in as `presented ?? credentials.credentialFor(...)` turned that test and only that test red (`1 failed | 8 passed`, the failure showing the presented key travelling in place of the held one) and was restored byte-identical. The source half wires in M4 alongside the first client.
- [x] Digest agreement check passes; **fails** when the transport's embedded digest is altered. Checkable: `node tools/digest-agreement.mjs` passes on a clean tree; alter one hex digit of the golden's embedded `SCHEMA_DIGEST` and it exits non-zero. (Ticked 2026-08-21: clean run `digest-agreement: schema 8835157bb157..., 2 artifact(s) agree`; the red was *run*, not described — one digit changed in `protocol/golden/src/index.ts` produced `digest-agreement: ... was cut from digest ... but the schema on disk is ...` and `1 problem(s)` on stderr with exit 1, and the restore was verified byte-identical by an empty `git diff` on the golden. Noted while running it: the red message truncated both digests to twelve characters, so a one-digit difference printed as two identical-looking strings — the check was sound and its message could not show what it found. Fixed on the failure path only, and the red re-run to confirm it: the message now prints `...a798 but the schema on disk is ...a797` in full, while the passing line stays short.)
- [x] A text-only integration test drives a real daemon to read a real element. Checkable: `apps/hub/src/__tests__/drives-a-real-daemon.test.ts` spawns the **built** daemon as a separate process over a real unix socket. (Ticked 2026-08-21: 3 tests passed; the daemon is spawned from `daemon/dist/main.mjs` with `--grant yad`, because a deny-by-default daemon with no grant answers an empty list and every assertion below it would pass on nothing. **Which leg is the test is said plainly, because presenting a tape as a live measurement is the failure worth preventing:** the *test* reads the committed `gtk-dialog` capture — a recording of a real accessibility tree taken from a live yad dialog, not a synthetic fixture — through the replay backend, which is what makes it deterministic in CI. The live leg against a running desk is a transcript beside the plan and is not this test; PR #38.)
- [x] Proof artifact: `which-credential-the-voice-lane-accepts.md`. (Ticked 2026-08-21: [proofs/which-credential-the-voice-lane-accepts.md](proofs/which-credential-the-voice-lane-accepts.md), added by the segment-5 PR. Four legs against the real provider on minibeast — a dial completing a turn on a minted token with two dials producing two different tokens, and three refusals that each name which failure they were: `409 NO_GOOGLE_ACCOUNT`, `401 CREDENTIAL_REJECTED` unretried, and a token seen expiring on the *close* rather than the dial after a real 150-second wait. Red leg: the same script against a merge-base build, `voice-lane: ABSENT from this build`. Every line the document quotes was checked to exist verbatim in the artifact it names. The three gaps it does not establish are stated in the document itself.)
- [x] The audit log for that run names every element the daemon **answered** and nothing else. Checkable: run box 3's test and read the audit file it wrote. (Ticked 2026-08-21: captured from that test's own run with the test unmodified, one line, one entry — `{"at":"2026-08-21T14:53:01.707Z","application":null,"element":[{"id":"el-a174b78401c1","role":"label"},{"id":"el-47577e4569ef","role":"button"}],"scope":"observe","cause":{"attribution":"external"},"attestation":null,"outcome":"read"}`. Exactly the two elements the run answered, each carrying identity only — keys exactly `id` and `role`, no name, no value, no text. The absence half is guarded by the daemon's own leak detector run over the tape the test read, so it cannot decay into a stale term list, and by the mutation `the-receipt-that-names-the-content` — deleting the `.map(identityOf)` narrowing in `daemon/src/audit.ts` took three tests red in `the-audit-log-names-what-was-touched.test.ts`, re-run for this tick. The record and its mutation shipped in PR #37; the test that produced this line is from PR #38. **The first version of this tick cited `the-memory-that-keeps-the-content` instead — a real mutation, but the one guarding the *hub's* subconscious view in `apps/hub/src/memory/stripped.ts`, not the daemon's record.** Both narrowings exist and both are guarded; the citation pointed at the wrong package, which the whole-feature review caught.) **This box read *touched* until 2026-08-21, and the code has always said *answered*.** A `queryElements` walk reads up to 2500 nodes, so a record of everything touched would hold the name of every element on the desk — the opposite of what [ADR-0026](02-DECISIONS/0026-the-audit-log-is-an-access-record-episodes-are-the-narrative.md) exists for. The divergence was disclosed here as a footnote while M3 was open, because rewording a milestone's own exit criterion mid-flight is how a gate quietly becomes whatever was built. With M3 closed and merged the honest artifact is a corrected box: the wording now matches the guarantee, and [ADR-0050](02-DECISIONS/0050-the-record-names-the-refusal-not-the-sentence.md) holds the reasoning and the history.
...
- [x] `xwininfo` reports the window managed (not override-redirect). **Ticked 2026-08-22:** box 1 in [the desk artifact](proofs/what-the-face-does-on-a-real-desk.md) reports `Override Redirect State: no` from the X server.
- [x] `xprop` shows the always-on-top state and the window present in the stacking list. **Ticked 2026-08-22:** box 2 records `_NET_WM_STATE_ABOVE`, membership in `_NET_CLIENT_LIST`, and that showing the face did not activate it.
...
- [x] A click on the orb sends a gesture; clicks in the transparent region send nothing. **Ticked 2026-08-22, split witness:** box 5 reads the smaller input shape from X; `apps/widget/src/__tests__/clicks-land-only-on-the-face.test.ts` proves the orb, caption and menu are inside that shape and transparent points are outside it.
- [x] Disabling from the tray unmaps the window and removes it from the window list. **Ticked 2026-08-22:** box 6 enters the tray-bound dismissal function without synthetic input and observes the window leave `_NET_CLIENT_LIST`.
- [x] The face stays visible for the whole of a long `progress` sequence. **Ticked 2026-08-22, split witness:** `apps/widget/src/__tests__/the-face-hides-when-told.test.ts` advances an injected clock by 24 hours and keeps progress visible; the same suite proves no timer can reach a hide, while the live desk exercises the built widget's visibility path.
...
- [ ] The daemon's audit log for that run shows the mail client and nothing else. (The hub does not write this record and never has: it is written by the daemon at the point of effect, per [ADR-0026](02-DECISIONS/0026-the-audit-log-is-an-access-record-episodes-are-the-narrative.md). Corrected 2026-08-21, before the milestone it belongs to opened.)
