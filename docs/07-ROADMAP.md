...
- [x] Every mutation entry deletes exactly the line it names. Checkable: a test asserts each entry's `find` string occurs exactly once in its target file. (Ticked 2026-08-20: `tools/mutations.mjs` refuses ambiguous and stale find strings with distinct exits, guarded by `tools/__tests__/mutations-runner.test.mjs`; full run on the final tree hit no such exit; PR #28.)
