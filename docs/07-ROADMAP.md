...
**Stage 2 delivered — operator-owned Gmail authority composition.** [ADR-0054](02-DECISIONS/0054-gmail-authority-is-composed-by-the-operator-unit.md) freezes the built-in `gmail` identity and the ownership split. A fresh installation seeds restrictive operator grants and capabilities, installs the complete daemon module tree, and composes the user unit to launch only `gmail`, observe `{gmail, chrome}` through the recipe's `appearsAs` join, enable durable launch only for Gmail, and write receipts only to an explicit protected audit path. Operator edits survive later applies. The unit remains unenabled, no application is launched, no Gmail content is traversed, and no model-controlled launch seam exists; those belong to later stages.
