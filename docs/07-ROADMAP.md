...
- [x] The audit log for that run names every element touched and nothing else. **The code records what the daemon *answered*, not what it *walked*** — see the footnote. Checkable: run box 3's test and read the audit file it wrote. (Ticked 2026-08-21: captured from that test's own run with the test unmodified, one line, one entry — `{"at":"2026-08-21T14:53:01.707Z","application":null,"element":[{"id":"el-a174b78401c1","role":"label"},{"id":"el-47577e4569ef","role":"button"}],"scope":"observe","cause":{"attribution":"external"},"attestation":null,"outcome":"read"}`. Exactly the two elements the run answered, each carrying identity only — keys exactly `id` and `role`, no name, no value, no text. The absence half is guarded by the daemon's own leak detector run over the tape the test read, so it cannot decay into a stale term list, and by the mutation `the-receipt-that-names-the-content` — deleting the `.map(identityOf)` narrowing in `daemon/src/audit.ts` took three tests red in `the-audit-log-names-what-was-touched.test.ts`, re-run for this tick. The record and its mutation shipped in PR #37; the test that produced this line is from PR #38. **The first version of this tick cited `the-memory-that-keeps-the-content` instead — a real mutation, but the one guarding the *hub's* subconscious view in `apps/hub/src/memory/stripped.ts`, not the daemon's record.** Both narrowings exist and both are guarded; the citation pointed at the wrong package, which the whole-feature review caught.) **The divergence, stated rather than smoothed:** this box says *touched* and the record says *answered*. A `queryElements` walk reads up to 2500 nodes, so recording everything touched would put the name of every element on the desk into the record — the opposite of what [ADR-0026](02-DECISIONS/0026-the-audit-log-is-an-access-record-episodes-are-the-narrative.md) is for. [ADR-0050](02-DECISIONS/0050-the-record-names-the-refusal-not-the-sentence.md) records the choice; the box's wording is kept as written and the difference is disclosed here rather than reworded away.
