...
**Scopes.** Four, and they are the vocabulary everywhere: `observe`, `edit`, `activate`, `submit`. They are ordered by consequence, not by convenience. `submit` — the class that sends the email, spends the money, posts the comment — is the one that requires an attestation the agent cannot author itself: the service refuses a commit it cannot describe (`ATTESTATION_FAILED`, protocol method `attestElement`/`commitElement`).
