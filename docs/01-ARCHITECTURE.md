...
**B3 is half-wired, and the half that is missing is named.** Its runtime half is real: a credential a caller presents is refused rather than honoured ([ADR-0007](02-DECISIONS/0007-identity-is-derived-credentials-are-minted.md)), tested in `apps/hub/src/__tests__/the-voice-lane-mints-one-token.test.ts`. Its source half has nothing to scan — a pin walking `apps/**` minus the hub walks an empty set and trips its own vacuity guard — so it wires in M4 alongside the first client. B4 and B9 remain absent for the same reason, stated plainly: there is no client yet to hold a microphone or a transcriber. [tools/pins/README.md](../tools/pins/README.md) records each wiring and each absence with its reason.
