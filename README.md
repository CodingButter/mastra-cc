...
**The next work is [M6 — The north star](docs/07-ROADMAP.md)**, starting with Stage 1: writing down, in one authoritative place, what *"tell me my most recent email"* means — which mailbox, what counts as proof of recency, what is spoken, what is audited, and how it refuses — before a line of it is implemented. The habit M1 set holds for every milestone after it — **make each gate fail on purpose before trusting it**. The prototype's freeze was prose; the schema changed 23 times afterwards and nothing ever failed. A gate that has only ever passed is indistinguishable from one that is not wired up.
