...
// Mastra CC protocol v1.13.1

export const PROTOCOL_VERSION = "1.13.1";
export const SCHEMA_DIGEST = "f0f191398650dfc3c9bc8b00ac2df75e9cc6796e099b9b0251ef6870cab62c04";
...
export interface ObservableContentTextWindow {
  ...
  totalLength: number;
  /** One-based line number containing the first character in value, when all line metadata can be determined without widening the content read. */
  startLine?: number;
  /** One-based line number containing the final character in value, or startLine for an empty window, when all line metadata can be determined without widening the content read. */
  endLine?: number;
  /** Total number of lines in the element's current text, when all line metadata can be determined without widening the content read. */
  totalLines?: number;
}
