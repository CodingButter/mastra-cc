...
// Mastra CC protocol v1.29.0

export const PROTOCOL_VERSION = "1.29.0";
export const SCHEMA_DIGEST = "7123f4e95a1d0f5c4226940e344fcce37f87e87538733b8d954b136958ee6b0f";
...
export const REFUSAL_OWNERS = ["agent","world","daemon"] as const;
export type RefusalOwner = (typeof REFUSAL_OWNERS)[number];
export const REFUSAL_CODES = ["AttestationFailedError","EffectUnsupportedError","KeyboardHeldElsewhereError","MagnitudeOutOfRangeError","OperationNotExposedError","PointerBlockedError","RecordingNotPerformableError","TextOffsetOutOfRangeError","UnperformableElementError","UnpublishedActionError","WriteNotObservedError","AccessibilityNotAcquirable","AccessibilityNotAcquired","AccessibilityLostMidSession","AlreadyRunning","BackendUnreadable","CouldNotStart","DeadlineExceeded","BlockedByDialog","DisabledByConfiguration","EffectClassGate","ElementGone","EnforcementUnrepresentable","InventoryUnsupported","LaunchUnavailable","MalformedParameter","NoConnection","NoMatch","RestartNotConfirmed","RestartNotOurs","RestartRefusedByApplication","NoRecipe","NotReadableInTime","OneBrowserIdentity","UnknownElement","UnknownMethod","UnknownSubscription","WatchDeaf","WatchUnknownElement","WatchUnsupported","ApplicationGone","DriverBusy","DriverClosed","WindowScopeUnmatched","WindowScopeAmbiguous","ApplicationScopeUnmatched","ApplicationScopeAmbiguous","ApplicationIdentityMismatch","AuditUnwritable","Unclassified"] as const;
...
/** Why the daemon did not do what was asked (ADR-0113). Every refusal says whose it is: agent (the request was malformed, stale, unknown, or beyond what the operator allowed; asking differently can succeed), world (the desktop or application said no, went away, froze, or changed under the call; the daemon reported it faithfully), or daemon (this daemon failed its own contract). A refusal always travels inside a response's result, never beside it. */
export interface Refusal {
  /** Whose refusal it is: agent, world, or daemon. */
  class: RefusalOwner;
  /** The stable name of the refusal. Branch on this, not on the message. */
  code: RefusalCode;
  /** The sentence a person reads: what was refused and, where known, what to do instead. */
  message: string;
  /** What the daemon saw that decided the refusal, when that differs from what was asked. */
  observed?: string;
  /** A suggested next move, when the daemon knows one. */
  next?: string;
}
...
export interface CapturedImage {
  ...
  format: string;
  /** The width of the returned picture in pixels. The picture may be clipped to the captured display and need not span the full element rectangle; clipped and crop say so. */
  width: number;
  /** The height of the returned picture in pixels. The picture may be clipped to the captured display and need not span the full element rectangle; clipped and crop say so. */
  height: number;
  ...
  data: string;
  /** Where the pixels came from: the desktop's visible pixels, always. Whatever was drawn over the element is in the picture; nothing here proves the element owns them, and no occlusion is established or ruled out. */
  source: "visible-desktop";
  /** True when the picture is less than the whole element: the display did not cover all of its rectangle. A clipped picture's centre is not the element's centre; crop says where in the element the picture lies. */
  clipped: boolean;
  /** Where this picture lies within the element, as fractions of the element's own rectangle. An unclipped picture is 0, 0, 1, 1. A location (u, v) inside the picture, each 0 through 1, is at element fraction (x + u * width, y + v * height) - the fractions clickElement takes. */
  crop: CaptureCrop;
  /** When the pixels were read, milliseconds since the Unix epoch on the daemon's clock. Bounds observation and capture are not atomic; this dates the picture, it does not promise the element still looks like it. */
  capturedAt: number;
}

/** A rectangle inside an element, named as fractions of that element's own width and height from its top-left corner. Every value is finite and within 0 through 1, and x + width and y + height do not exceed 1. This is a capture-time relationship, not a lasting coordinate: the element may move or change after the picture, so resolve it again before acting on it. */
export interface CaptureCrop {
  /** The picture's left edge as a fraction of the element's width from its left edge. */
  x: number;
  /** The picture's top edge as a fraction of the element's height from its top edge. */
  y: number;
  /** The picture's width as a fraction of the element's width. */
  width: number;
  /** The picture's height as a fraction of the element's height. */
  height: number;
}
...
export interface CaptureElementResult {
  /** Present when the picture was taken: currently visible pixels intersecting the element rectangle and captured display. It may be clipped, and it may show other windows drawn over the element; clipped and crop say which part of the element it is, so a location in the picture can be mapped to the fractions clickElement takes. */
  image?: CapturedImage;
