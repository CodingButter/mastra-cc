...
export async function handleRequest(
  ...
  try {
    ...
    // not complete is a fact about access. A walk that exhausted its budget is
    // a refusal the daemon can name; every other throw remains a failed attempt.
    // Subclasses here do not set .name (IncompleteObservationError reads as
    // "Error" through it), so the class's constructor name is what gets written.
