...
export async function captureWakeAudio(signal?: AbortSignal): Promise<Buffer> {
  return await createMicrophoneCapture(
    { device: process.env.MASTRA_CC_MICROPHONE_DEVICE ?? "plughw:0,6", seconds: 2 },
    signal,
  );
}

export function adaptEnrolmentCapture(raw: Buffer): ProcessedCapture {
  return processEnrolmentCapture(raw, WIDGET_CAPTURE_FORMAT);
}
